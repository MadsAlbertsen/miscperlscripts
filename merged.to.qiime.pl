...
#    merged.to.qiime.pl
#
#    Reformats a merged fasta into qiime ready format
...
#
#    ..also.. the reorder function stores the complete sample in memory.. 
#    Reordering is needed for scrambled data.
#
...
############################ Reorder the input file if it has been scrambled #####################

if ($re > 0){
open(tempOUT, ">reordered.fa") or die("Cannot create file: reordered.fa\n");

print "Reordering the input file.\n";	
...
while (my $line = <INreads>)  {	                                                                   
	...
	if ($headernr == 0) {
		...
		my $id1 = "$splitline[-1]-$splitline[2]"; 
		my $id2 = "$header $line";
		$reorder{$id1}{$id2} = 1;		
	}
}

for my $sample ( sort keys %reorder ) {
			for my $read ( keys %{ $reorder{$sample} } ) {
				my @splitline = split(/ /, $read);
				print tempOUT "$splitline[0]\n$splitline[1]\n";
			}
		}
...
close INreads;
close tempOUT;
open(INreads, "reordered.fa") or die("Cannot open: reordered.fa\n");
}
...
sub checkParams {
    ...
    #
    my @standard_options = ( "help|h+", "infile|i:s","sampleidfile|s:s","subsample|m:s","badout|b:+","unique|u:s","percent|p:s","re|r:+");
    my %options;
...
script.pl  -i -s [-h -m -b -u -p]
 ...
 [-subsample -m]      Only use the first X reads (default: all).
 [-badout -b]         Print non sampleid sequences to a file (default: off).
 [-unique -u]         Remove sequence < X (default: 0).
 [-percent -p]        Percentage filter - Experimental feature (default: 0).
 [-reorder -r]        Reorder the input file if it is not ordered by sample (default: off).
